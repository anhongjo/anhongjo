...
import java.awt.event.*;
 ...
 protected boolean value;
  ...
  addKeyListener(this);
  colors = new Color[10]; // 테트리스 배경 및 조각 색
 ...
 public void paint(Graphics g) {
	  super.paint(g);
	     ...
	     g.draw3DRect(300, 230, 130, 40, true);
	        g.setFont(new Font(null, Font.BOLD, 20));//점수 표시하기
	         g.drawString(" " + score,350, 255);
	     g.drawString("점  수", 340, 220);
	 ...
	 public void run(){
		  int random, random2;
		  random2 = (int)(Math.random() * Integer.MAX_VALUE) % 8;
		  
	  while(!stop) {
	  try {
	  if(makeNew){ // 새로운 테트리스 조각 만들기
		  if (value == false) {
			  random = (int)(Math.random() * Integer.MAX_VALUE) % 8;
			  value = true;
		  } else {
			  random = random2;
			  random2 = (int)(Math.random() * Integer.MAX_VALUE) % 8;
